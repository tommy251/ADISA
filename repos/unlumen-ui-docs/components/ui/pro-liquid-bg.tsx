...
/* ─────────────────────────────────────────────────────────────────────────────
   ProLiquidBg
   Full Navier-Stokes fluid simulation on black background with:
   - Pink / magenta fluid palette
   - Bayer 4×4 ordered dithering (DataTexture)
   - Pixelation step in the output pass
   - Animated film-grain noise
   - Auto-demo mode (fluid moves on its own, reacts to cursor when hovered)
   ───────────────────────────────────────────────────────────────────────────── */
...
// Output shader: pixelation + Bayer dithering + heavy film grain
...
void main(){
  ...
  float dither  = texture2D(uBayer, bayerUV).r - 0.5; // [-0.5, 0.5]

  // Subtle noisy displacement on the palette sample
...
// Dark-mode fluid palette: deep purples → hot pink → soft pink
const PALETTE_DARK = ["#000000", "#2a0020", "#8c0f60", "#e8227a", "#ff85b3"];
// Light-mode fluid palette: vivid magentas/purples that pop on white
const PALETTE_LIGHT = ["#ffffff", "#FD96E5", "#F36AC3", "#FE4396", "#ff85b3"];
...
/** bgColor.rgb = blend-to colour (black in dark mode, white in light mode).
 *  alpha stays 0 so no-fluid pixels remain fully transparent. */
function getBgColor() {
  return isDarkMode()
...
/** 4×4 Bayer ordered dithering matrix as a tiling DataTexture */
function makeBayerTexture(): THREE.DataTexture {
  // Standard 4×4 Bayer matrix values [0..15], normalized to [0..255]
  const raw = [
...
class CommonGL {
  ...
  init(container: HTMLElement) {
    ...
    // Cap at 1 — this is a background effect, no need for retina resolution
    this.pixelRatio = 1;
    this.resize();
...
class FluidSim {
  ...
  private _makeFBO() {
    // HalfFloat everywhere — velocity values are small, halves GPU memory bandwidth
  ...
  private _createFBOs() {
    const names = ["vel_0", "vel_1", "vel_v0", "vel_v1", "div", "p0", "p1"];
    for (const n of names) this.fbos[n] = this._makeFBO();
  }
  ...
  update(time: number) {
    ...
    void time; // suppress unused warning (reserved for future noise displacement)
  }
...
/** Fullscreen fluid background. No-fluid pixels are fully transparent so the
 *  parent's `bg-background` CSS class shows through — dark/light mode works. */
export function ProLiquidBg() {
  ...
  useEffect(() => {
    ...
    gl.init(container);
    // Canvas must be in the DOM for WebGL to render anything visible
    ...
    const palette = makePaletteTexture(
      isDarkMode() ? PALETTE_DARK : PALETTE_LIGHT,
    );
    ...
    const sim = new FluidSim(gl, mouse, {
      ...
      cursor_size: 110,
      dt: 0.008,
      BFECC: false, // 1 texture lookup vs 4 — imperceptible on a bg
      isBounce: false,
      isViscous: false,
      iterations_poisson: 8, // 8 is plenty for a decorative background
    });

    // Output pass (final render to screen)
    const outputUniforms: Uniforms = {
      velocity: { value: sim.fbos.vel_0!.texture },
      palette: { value: palette },
      uBayer: { value: bayerTex },
      // rgb = blend-to colour (black dark / white light); alpha=0 keeps no-fluid pixels transparent
      bgColor: { value: getBgColor() },
      uTime: { value: 0 },
      uRes: { value: new THREE.Vector2(gl.width, gl.height) },
      uPixelSize: { value: 18.0 }, // subtle pixel grid
      boundarySpace: { value: new THREE.Vector2() },
      px: { value: new THREE.Vector2() }, // required by face_vert
    };
    ...
    // Swap palette + bgColor blend when dark/light mode changes
    const themeObserver = new MutationObserver(() => {
      const stops = isDarkMode() ? PALETTE_DARK : PALETTE_LIGHT;
